...
\subsection{Modèle stochastique à mini-batch}

Le modèle d'apprentissage est modifié afin d’utiliser des \emph{mini-batches} de positions du jeu au lieu d’une seule position à chaque mise à jour.  
...
\]
où $\alpha_t$ est le taux d'apprentissage, ajusté à chaque époque par une règle de \emph{Step Decay} :
\[
\alpha_t = \alpha_0 \times 0.5^{\frac{t}{s}}
\]
avec $s$ le paramètre de pas de décroissance (\textit{step size}).
  
\end{definition}

\subsection{Mécanismes d'adaptation du taux d'apprentissage}
Deux stratégies sont combinées pour améliorer la stabilité :
\begin{itemize}
  \item \textbf{Step Decay} : réduction progressive du taux d’apprentissage selon une loi exponentielle ;
  \item \textbf{ReduceLROnPlateau} : réduction automatique du taux d’apprentissage lorsqu’aucune amélioration de la précision sur le jeu de validation n’est observée pendant plusieurs époques consécutives.
\end{itemize}
Ces mécanismes assurent un apprentissage plus régulier, évitant la divergence du modèle.
